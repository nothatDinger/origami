# Origami: Reclaiming GPU Time from KV Cache Decompression under Bandwidth-Constrained LLM Serving
...
For the complete environment setup and patch workflow, see
[`docs/install.md`](docs/install.md).

Apply Origami to a clean vLLM 0.18.0 source tree and install it:

```bash
git clone https://github.com/nothatDinger/origami.git Origami
...
For QAT checks, lossless validation, and connector usage examples, see
[`docs/usage.md`](docs/usage.md).

Run the QAT lossless path:

```bash
ORIGAMI_QAT_CODEC=qat_codec python3 scripts/test_lossless.py --backend qat
```
